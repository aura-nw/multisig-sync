  ...
  async handleQueryTxByHeight(job: Job) {
    ...
    while (!done) {
      try {
        ...
        if (resultCallApi.data[this.graphqlPrefix].transaction.length > 0) {
          resultCallApi.data[this.graphqlPrefix].transaction.map((res) => {
            result.push(res.data);
          });
          variables.id = resultCallApi.data[this.graphqlPrefix].transaction
            .map((tx) => tx.id)
            .sort((a, b) => b - a)[0];
        } else {
          done = true;
    ...
    try {
      if (result.length > 0) {
        await this.commonService.handleTransactions(result, safes, network);
      }
    } catch (error) {
      this.logger.error(error);
    }
