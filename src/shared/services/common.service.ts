    ...
    async handleTransactions(listTx, safes, chain) {
        ...
        try {
            listTx.map(txs => {
                ...
                let auraTxAmount = null, auraTxRewardAmount = null;
    
                txs.tx.body.messages.filter(msg =>
                    ...
                    switch (type) {
                        ...
                        case MESSAGE_ACTION.MSG_MULTI_SEND:
                            ...
                            msg.outputs.filter(output => safes[msg.inputs[0].address] || safes[output.address])
                                .map(output => {
                                    ...
                                    auraTxAmount += parseFloat(output.coins[0].amount);
                                    listTxMessages.push(txMessage);
                                });
                            break;
                            ...
                            if (txs.tx_response.logs.length > 0) {
                                if (txs.tx_response.logs[index].events.find(event => event.type === CONST_CHAR.WITHDRAW_REWARDS)) {
                                    txMessage.amount = txs.tx_response.logs[index]
                                        .events.find(event => event.type === CONST_CHAR.WITHDRAW_REWARDS).attributes
                                        .find(attr => attr.key === CONST_CHAR.AMOUNT).value.match(/\d+/g)[0];
                                    auraTxRewardAmount += parseFloat(txMessage.amount);
                                }
                            }
                            listTxMessages.push(txMessage);
                            break;
                        case MESSAGE_ACTION.MSG_REDELEGATE:
                            ...
                            if (txs.tx_response.logs.length > 0) {
                                if (txs.tx_response.logs[index].events.find(event => event.type === CONST_CHAR.WITHDRAW_REWARDS)) {
                                    txMessage.fromAddress = txs.tx_response.logs[index]
                                        .events.find(event => event.type === CONST_CHAR.WITHDRAW_REWARDS).attributes
                                        .find(attr => attr.key === CONST_CHAR.VALIDATOR).value;
                                    txMessage.amount = txs.tx_response.logs[index]
                                        .events.find(event => event.type === CONST_CHAR.WITHDRAW_REWARDS).attributes
                                        .find(attr => attr.key === CONST_CHAR.AMOUNT).value.match(/\d+/g)[0];
                                    auraTxRewardAmount += parseFloat(txMessage.amount);
                                    withdraw_rewards = txs.tx_response.logs[index]
                                        .events.find(event => event.type === CONST_CHAR.WITHDRAW_REWARDS);
                                }
                            }
                            listTxMessages.push(txMessage);
                            if (withdraw_rewards) {
                                if (withdraw_rewards.attributes.length > 2) {
                                    let txMessageDst = new Message();
                                    txMessageDst.typeUrl = MESSAGE_ACTION.MSG_REDELEGATE;
                                    txMessageDst.toAddress = msg.delegator_address;
                                    txMessageDst.fromAddress = txs.tx_response.logs[index]
                                        .events.find(event => event.type === CONST_CHAR.WITHDRAW_REWARDS).attributes[3].value;
                                    txMessageDst.amount = txs.tx_response.logs[index]
                                        .events.find(event => event.type === CONST_CHAR.WITHDRAW_REWARDS)
                                        .attributes[2].value.match(/\d+/g)[0];
                                    auraTxRewardAmount += parseFloat(txMessageDst.amount);
                ...
                if (listTxMessages.length > 0) {
                    ...
                    auraTx.fromAddress = this.listMessageStake.includes(listTxMessages[0].typeUrl)
                        ? listTxMessages[0].toAddress
                        : listTxMessages[0].fromAddress;
                    auraTx.toAddress = this.listMessageStake.includes(listTxMessages[0].typeUrl)
                        ? null
                        : listTxMessages[0].toAddress;
