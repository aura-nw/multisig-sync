    ...
    try {
      listTx.map((txs) => {
          ...
          switch (type) {
            ...
            case MESSAGE_ACTION.MSG_MULTI_SEND:
              ...
              msg.outputs
                .filter(
                  (output) =>
                    safes[msg.inputs[0].address] || safes[output.address],
                )
                .map((output) => {
              ...
              relatedSafeAddress = [
                ...msg.inputs.map((input: Input) => input.address),
                ...msg.outputs.map((output: Output) => output.address),
            ...
            case MESSAGE_ACTION.MSG_DELEGATE:
              ...
              if (txs.tx_response.logs.length > 0) {
                if (
                  ...
                  txMessage.amount = txs.tx_response.logs[index].events
                    .find((event) => event.type === CONST_CHAR.WITHDRAW_REWARDS)
                    .attributes.find((attr) => attr.key === CONST_CHAR.AMOUNT)
                    .value.match(/\d+/g)[0];
            ...
            case MESSAGE_ACTION.MSG_REDELEGATE:
              ...
              if (txs.tx_response.logs.length > 0) {
                if (
                  ...
                  txMessage.fromAddress = txs.tx_response.logs[index].events
                    .find((event) => event.type === CONST_CHAR.WITHDRAW_REWARDS)
                    .attributes.find(
                      (attr) => attr.key === CONST_CHAR.VALIDATOR,
                    ).value;
                  txMessage.amount = txs.tx_response.logs[index].events
                    .find((event) => event.type === CONST_CHAR.WITHDRAW_REWARDS)
                    .attributes.find((attr) => attr.key === CONST_CHAR.AMOUNT)
                    .value.match(/\d+/g)[0];
                  auraTxRewardAmount += parseFloat(txMessage.amount);
                  withdraw_rewards = txs.tx_response.logs[index].events.find(
                    (event) => event.type === CONST_CHAR.WITHDRAW_REWARDS,
                  );
                }
              }
            ...
            case MESSAGE_ACTION.IBC_RECEIVE:
              ...
              if (eventLog) {
                ...
                relatedSafeAddress.push(
                  ...[sender, recipient].filter((address) => safes[address]),
                );
                if (relatedSafeAddress.length === 0) break;
        ...
        if (listTxMessages.length > 0) {
          ...
          auraTx.rewardAmount = auraTxRewardAmount || null;
          auraTx.denom = chain.denom;
          auraTx.contractAddress = listTxMessages[0].contractAddress || null;
      ...
      if (syncTxs.length > 0) {
        const txs = await this.auraTxRepository.insertBulkTransaction(syncTxs);

        // Insert tx history
        await this.txHistoryRepository.create(txsHistory);

        syncTxMessages.map((txMessage, index) => {
          txMessage.map((tm) => (tm.auraTxId = txs[index].id));
        });
